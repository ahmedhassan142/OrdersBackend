...
const getRecipientEmail = async (order: IOrder): Promise<string> => {
  try {
    ...
    if (order.userId) {
      const email = await getUserEmail(order.userId.toString());
...
export const getOrderConfirmationContent = async (order: IOrder) => {
  try {
    ...
    const itemsWithDetails = await Promise.all(
      order.items.map(async (item) => {
        try {
          ...
          const response = await productService.get(`/api/products/${item.productId}`);
          ...
          return {
            ...
            size: item.size,
            color: item.color,
            image: product?.imageUrl // Assuming products have images array
          };
        } catch (error) {
    ...
    const recipientEmail = await getRecipientEmail(order);
